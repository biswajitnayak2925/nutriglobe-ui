...
main.py
-------
FastAPI backend that exposes data_processing / recommendation / comparison
/ input_resolver as a REST API for the frontend.

Run:
    uvicorn main:app --reload --port 8000

Then, ONCE, before using search/recommend/compare:
    curl -X POST http://localhost:8000/api/build
(this fetches data from Open Food Facts and builds embeddings — slow,
run it once, not per request. Re-run it whenever you want fresh data.)
"""
...
# Defaults to "*" (open) for local testing — tighten this before going live.
...
# ------------------------------------------------------------------
# Engines are loaded ONCE at startup (singletons), not per-request.
# If the dataset hasn't been built yet, they stay None until /api/build runs.
# ------------------------------------------------------------------
...
def _clean(obj):
    """Recursively convert NaN / numpy scalars into JSON-safe values."""
...
@app.post("/api/build")
def build():
    """Rebuilds the dataset from Open Food Facts + recomputes embeddings.
    SLOW (can take several minutes) — run manually once, not from the UI
    on every page load."""
...
def add_product(barcode: str):
    """Fetch ONE product by barcode from Open Food Facts and add it to the
    existing dataset — use this to backfill a specific missing product
    (e.g. a Coca-Cola barcode) without rebuilding everything."""
    try:
    ...
    _load_engines()  # reload so the new product is searchable immediately
    return result
...
async def compare(
    ...
    """Accepts EXACTLY ONE of:
      - `product_id` (form field): compare a SPECIFIC known product_id
        (use this when the user clicked a specific search result — avoids
        ambiguity between near-duplicate products with the same name)
      - `input` (form field): a barcode string or a free-text product name
      - `image` (file upload): a photo of the product / its barcode
    """
    engine = _require_compare()
